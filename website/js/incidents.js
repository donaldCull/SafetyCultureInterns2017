...
function create_menu() {
    ...
    for (var i = 0; i < sensors.length; i++){
        ...
        document.getElementById("reports_menu").innerHTML += "<a onclick='on_sensor_click(this)' href=\"" + link + "\" " +
            "class=\"list-group-item list-group-item-action bg-light border-primary small text-primary\" id=\"" + id + "\">" + label + "</a>";
    }
...
function on_sensor_click(this_object) {
    ...
    document.getElementById("reports_menu").innerHTML += "<a onclick='create_menu()' " +
        " class=\"list-group-item list-group-item-action small bg-light border-primary text-primary \">Back</a>"
    for (var i = 0; i < raw_data.length; i++){
        if (raw_data[i].incid_serial === current_sensor){
            ...
            document.getElementById("reports_menu").innerHTML += "<a onclick='on_report_click(this)' " +
                "href=\"" + link + "\" class=\"list-group-item small list-group-item-action bg-light border-primary text-primary \" id=\"" + id + "\">" + label + "</a>"
        }
    }
}
